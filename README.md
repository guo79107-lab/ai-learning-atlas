...
比赛方向：基于知乎问答和专业讨论的互动学习体验。三级每关第三个页签为“观点练习”，含 11 个与课程对应的原创情境题，始终标注非知乎原文。用户也可以自行添加知乎回答/专栏链接、标题、作者和相关摘录；网站不自动抓取链接，不伪造知乎材料。

练习流程：读材料 → 选择或提出追问 → 提炼观点、记录依据、反问 → 修订判断 → 保存观点卡 → 四级复盘。缺少证据可明确写待核查；保存只代表填写完成。自定义材料须有有效知乎链接、标题、摘录和四步记录才能完成。链接限制 http(s) 及明确知乎域名，所有文本转义渲染。记录存于原进度对象的 metrics.discussions，旧进度不变，示例与知乎材料各自保留独立草稿和来源，修改后重新标记草稿，观点练习不混入客观测验正确率。

四级把来源与四步判断排成有序图解，可回到原文、继续编辑或连同原有笔记一起导出 Markdown。个人任务的四步工作区保留在可展开区域，避免首次打开时信息过密。所有摘录和练习仅保存在当前浏览器设备；没有跨设备同步、模型评分或平台认证。

## 部署

已上线：[https://47.93.230.221/ai-learning/](https://47.93.230.221/ai-learning/)。当前发布 ID 为 `atlas-20260909-r7`。
