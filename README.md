...
- 早期视频方案的素材保留归档，旧版光圈视频已停用；首页使用即梦生成的晴日草坪共读背景视频。原始地址及光圈测量过程记录在海报目录中的 `视频光圈节奏.json`。缩放曲线基于 0.25 秒采样的可见青色外圈上缘，不是完整椭圆的物理半径。视频持续放大，没有缩小回程；循环淡出后重置。网页视频保留原始分辨率与时长，采用 H.264 CRF20 与 faststart 优化，从约 19 MB 缩至约 7.4 MB，原始文件保留；逐帧 SSIM 为 0.993。
...
已上线：[https://47.93.230.221/ai-learning/](https://47.93.230.221/ai-learning/)。当前发布 ID 为 `atlas-20260909-r11`。
...
最终素材与一份合并提示词位于 `/Users/agnet/AI学习海报-20260909/首页视频-最终版/`。内置作图完成首帧，书页展示“学会提问”和“独立思考”；即梦 Seedance 2.0 VIP 按用户指定生成 5 秒、1080P 视频，人物微动、草叶稍明显地轻摆。网页副本同站提供，使用静音循环、封面兜底，支持暂停；减少动态效果偏好和后台标签页会暂停播放。

首页恢复加视频前的居中标题、文案、按钮排版。影片以绝对定位铺满背景，不占据内容排版空间。11 本书籍流水线默认不渲染；点击“展开 11 本学习图鉴”才加载显示，再点收起，刷新或重新进入首页后仍默认隐藏。展开不推动文字位置，减少动态效果偏好下展开按钮仍可使用。所有首页学习入口仍仅进入 `/explore`，第二到第四级页面不变。网页视频以 120 帧 / 24fps 输出 5 秒，保留 1920×1080，移除音轨并优化首屏播放。4 组废弃首页试图及其提示词已按用户要求移入废纸篓。
