...
- 早期视频方案的素材保留归档，旧版光圈视频已停用；r10 首页改为即梦生成的晴日草坪共读视频。原始地址及光圈测量过程记录在海报目录中的 `视频光圈节奏.json`。缩放曲线基于 0.25 秒采样的可见青色外圈上缘，不是完整椭圆的物理半径。视频持续放大，没有缩小回程；循环淡出后重置。网页视频保留原始分辨率与时长，采用 H.264 CRF20 与 faststart 优化，从约 19 MB 缩至约 7.4 MB，原始文件保留；逐帧 SSIM 为 0.993。
...
已上线：[https://47.93.230.221/ai-learning/](https://47.93.230.221/ai-learning/)。当前发布 ID 为 `atlas-20260909-r10`。
...
桌面首页文案在左、影片在右，手机完整展示横版画面，11 本书籍流水线在影片下方继续保留。所有首页学习入口仍仅进入 `/explore`，第二到第四级页面不变。网页视频以 120 帧 / 24fps 输出 5 秒，保留 1920×1080，移除音轨并优化首屏播放。4 组废弃首页试图及其提示词已按用户要求移入废纸篓。
