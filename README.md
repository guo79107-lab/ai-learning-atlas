...
**首页 → 星球学习地图 → 11 关课程 → 学习复盘**。首页的视频只是背景；标题采用加粗白色系统字体，11 本立体书默认隐藏，点击才展开。所有首页学习入口统一进入第二级地图，再由「进入学习」进入课程。
...
- AI 陪练：填写真实目标，先独立作答，再由 **DeepSeek V4 Pro** 指出一个具体缺口，引用原答片段，按依据意识、推理过程、适用边界给反馈，并给出新情境题、方法卡和推荐关卡。
- 学习复盘：查看真实活跃时长、判断题记录、陪练历史和复习卡。首次练习 1 天后回顾；自评能说清后分别间隔 3 天、7 天回顾。不熟悉则回到 1 天。回顾必须写下自己的理解，可延期。

AI 反馈不代表事实已被证实，也不代表智力、专注力或长期掌握度。推荐关卡依据本次反馈的缺口，不凭访问次数生成能力分数。课程判断题的完成标记与 AI 陪练记录彼此独立。
...
在独立终端配置上述变量后运行 `python3 server/coach.py`。没有配置 AI 时，静态课程、固定练习、笔记与复盘仍可使用；陪练请求显示明确错误，不伪造反馈。
...
前端技术栈：React 19、TypeScript、Vinext/Vite、Tailwind CSS、Lucide、Recharts。`dist/client` 为静态产物，包含 14 个页面和对应 RSC 数据。没有新增 UI 依赖。Python API 只使用标准库，独立于静态部署。
...
- 原课程进度保持在 `ai-learning-atlas:v1`；新增陪练独立保存在 `ai-learning-atlas:coach:v1`，不覆盖旧笔记或完成记录。
- 草稿、本次回答、AI 反馈、回顾记录保存在当前浏览器，最多保留最近 100 次陪练。可在复盘页导出 Markdown；本页存储不可用时可直接导出当前内容。没有云端账号同步。
- 只有点击提交，才将本次目标、课程材料、原答（修订时）与回答发送给 DeepSeek。不要输入敏感个人信息。服务器不持久保存学习文本；成功结果在内存缓存 5 分钟供重试复用。SQLite 仅保存按日计数和经 HMAC 处理的 IP 标识。
- 固定官方 API `https://api.deepseek.com/chat/completions`，固定模型 `deepseek-v4-pro`，关闭思考模式，JSON 输出，最多 2600 输出 tokens。客户端不能指定模型、上游地址或系统提示。
...
- 不执行 AI 返回的 HTML，不抓取用户输入的 URL。摘录中的指令作为不可信材料处理。没有接入知乎 OAuth、知乎检索或外部事实核查，不声称已经搜索或证实。
...
- 首页晴日共读图片与 5 秒 1080P 视频为 AI 生成素材，视频由即梦生成。支持暂停、减少动态效果与后台暂停。
- 首页书籍排布复用用户授权参考的书籍流水线效果；海报使用用户的 11 张课程图。水星背景为用户星球项目的授权副本，原项目未修改。
- 苹果风格标题使用系统字体回退，保持加粗白色；按钮使用透明玻璃效果。不捆绑或分发苹果专有字体。
