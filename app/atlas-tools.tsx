  ...
  useEffect(() => {
    ...
    const tools: Tool[] = [
      {
        ...
        title: '查看学习进度',
        description:
          'Read completed chapter numbers and the last visited chapter on this device. Does not return private notes.',
        inputSchema: {
          type: 'object',
          properties: {},
        ...
        execute(input) {
          ...
          return { completed: p.completed, lastChapter: p.last, total: 11 };
        },
      },
      {
        ...
        execute(input) {
          if (
            ...
            typeof input !== 'object' ||
            Array.isArray(input) ||
            Object.keys(input).some((k) => k !== 'chapter')
          )
            throw new Error('Expected a chapter number');
          const id = (input as { chapter?: unknown }).chapter;
          if (
            typeof id !== 'number' ||
            !Number.isInteger(id) ||
            id < 1 ||
            ...
            throw new Error('Chapter must be 1–11');
    ...
    for (const tool of tools) {
      try {
        ...
        /* Unsupported registries do not affect the learning interface. */
      }
    }
