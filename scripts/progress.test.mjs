...
globalThis.window={};
globalThis.localStorage={getItem:k=>storage.get(k)??null,setItem(k,v){if(failWrites)throw new Error('Storage blocked');storage.set(k,v);}};
const {readProgress,normalizeProgress,saveNote,markComplete,rememberChapter}=await import('../app/atlas-store.ts');
...
test('preserves another tab’s saved work when this tab has a stale snapshot',()=>{
 ...
 storage.set(key,JSON.stringify({completed:[1],notes:{1:'另一页写下的发现'},last:2}));
 saveNote(2,'这页的新发现');
...
test('completion is idempotent and reading updates the resume position',()=>{
 markComplete(2);markComplete(2);rememberChapter(6);
...
test('malformed persisted values cannot create nonexistent chapters',()=>{
 const p=normalizeProgress({completed:[1,1,0,12,'2',3.5],notes:{1:5,2:'a'.repeat(3100)},last:99});
 assert.deepEqual(p.completed,[1]);assert.equal(p.last,1);assert.equal(p.notes[1],undefined);assert.equal(p.notes[2].length,3000);
});
